...
function createRouter2(init) {
  ...
  async function handleLoaders(request, location, matches, overrideNavigation, submission, fetcherSubmission, replace, initialHydration, flushSync, pendingActionData, pendingError) {
    ...
    revalidatingFetchers.forEach((rf) => fetchControllers.delete(rf.key));
    let redirect6 = findRedirect2(results);
    if (redirect6) {
      if (redirect6.idx >= matchesToLoad.length) {
        let fetcherKey = revalidatingFetchers[redirect6.idx - matchesToLoad.length].key;
        fetchRedirectIds.add(fetcherKey);
  ...
  async function startRedirectNavigation(state2, redirect6, _temp2) {
    ...
    let redirectLocation = createLocation2(state2.location, redirect6.location, {
      _isRedirect: true
    });
    ...
    if (isBrowser2) {
      ...
      if (redirect6.reloadDocument) {
        isDocumentReload = true;
      } else if (ABSOLUTE_URL_REGEX2.test(redirect6.location)) {
        const url = init.history.createURL(redirect6.location);
        isDocumentReload = // Hard reload if it's an absolute URL to a new origin
        url.origin !== routerWindow.location.origin || // Hard reload if it's an absolute URL that does not match our basename
    ...
    let activeSubmission = submission || fetcherSubmission;
    if (redirectPreserveMethodStatusCodes2.has(redirect6.status) && activeSubmission && isMutationMethod2(activeSubmission.formMethod)) {
      await startNavigation(redirectHistoryAction, redirectLocation, {
...
function persistAppliedTransitions2(_window, transitions) {
  if (transitions.size > 0) {
    ...
    try {
      _window.sessionStorage.setItem(TRANSITIONS_STORAGE_KEY2, JSON.stringify(json6));
    } catch (error) {
  ...
  "node_modules/react-router-dom/server.js"(exports) {
    ...
    function StaticRouterProvider2({
      ...
      if (hydrate !== false) {
        ...
        let json6 = htmlEscape(JSON.stringify(JSON.stringify(data)));
        hydrateScript = `window.__staticRouterHydrationData = JSON.parse(${json6});`;
      }
      let {
        state
...
function createRouter3(init) {
  ...
  async function handleLoaders(request, location, matches, overrideNavigation, submission, fetcherSubmission, replace, initialHydration, flushSync, pendingActionData, pendingError) {
    ...
    revalidatingFetchers.forEach((rf) => fetchControllers.delete(rf.key));
    let redirect6 = findRedirect3(results);
    if (redirect6) {
      if (redirect6.idx >= matchesToLoad.length) {
        let fetcherKey = revalidatingFetchers[redirect6.idx - matchesToLoad.length].key;
        fetchRedirectIds.add(fetcherKey);
  ...
  async function startRedirectNavigation(state2, redirect6, _temp2) {
    ...
    let redirectLocation = createLocation3(state2.location, redirect6.location, {
      _isRedirect: true
    });
    ...
    if (isBrowser2) {
      ...
      if (redirect6.reloadDocument) {
        isDocumentReload = true;
      } else if (ABSOLUTE_URL_REGEX4.test(redirect6.location)) {
        const url = init.history.createURL(redirect6.location);
        isDocumentReload = // Hard reload if it's an absolute URL to a new origin
        url.origin !== routerWindow.location.origin || // Hard reload if it's an absolute URL that does not match our basename
    ...
    let activeSubmission = submission || fetcherSubmission;
    if (redirectPreserveMethodStatusCodes3.has(redirect6.status) && activeSubmission && isMutationMethod3(activeSubmission.formMethod)) {
      await startNavigation(redirectHistoryAction, redirectLocation, {
...
function persistAppliedTransitions3(_window, transitions) {
  if (transitions.size > 0) {
    ...
    try {
      _window.sessionStorage.setItem(TRANSITIONS_STORAGE_KEY3, JSON.stringify(json6));
    } catch (error) {
...
@remix-run/react/dist/esm/index.js:
   ...
   *)
*/
//# sourceMappingURL=http://localhost:3100/mini-oxygen/00000/11111/22222/33333/build/_shared/chunk-N4WRVMA3.js.map
